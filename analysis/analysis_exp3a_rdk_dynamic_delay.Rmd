...
experimentName = "dynamic_rdk" # use the index.Rmd file to track which experiment is which

# Name of the data file containing raw data
datafilename = "2021_exp3a_rdk_data_dynamic.csv" # Name of data file
...
fu = makeHeatmap(selections, "fixed_moving_delay"  , "untimed" )
fd = makeHeatmap(selections, "fixed_moving_delay"  , "deadline" )
ru = makeHeatmap(selections, "random_moving_delay" , "untimed" )
rd = makeHeatmap(selections, "random_moving_delay" , "deadline" )
```
...
unitable = uniformityTest(selections, c("fixed_moving_delay", "random_moving_delay"), c("untimed", "deadline"), fu, fd, ru, rd) 
...
fu = selections %>% 
  filter(condition == "fixed_moving_delay", phase == "untimed") %>% 
...
fd = selections %>% 
  filter(condition == "fixed_moving_delay", phase == "deadline") %>% 
  select(subject, trial, selected_difficulty_1, selected_difficulty_2, selected_difficulty_3, selected_difficulty_4) %>%
  as.matrix() %>% 
  getPLparametersBySubject()

ru = selections %>% 
  filter(condition == "random_moving_delay", phase == "untimed") %>% 
  select(subject, trial, selected_difficulty_1, selected_difficulty_2, selected_difficulty_3, selected_difficulty_4) %>%
  as.matrix() %>% 
  getPLparametersBySubject()

rd = selections %>% 
  filter(condition == "random_moving_delay", phase == "deadline") %>% 
  select(subject, trial, selected_difficulty_1, selected_difficulty_2, selected_difficulty_3, selected_difficulty_4) %>%
  as.matrix() %>% 
  getPLparametersBySubject()
```
...
#files_to_build <- list.files("analysis", pattern = "^analysis_exp3a_.*\\.Rmd$", full.names = TRUE)
