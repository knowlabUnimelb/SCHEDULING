---
title: "Experiment 7e: Labelled RDK - Point Reward"
...
datafilename = "exp7e_rdk_reward_data_points.csv"
...
#uncleandata = rawdata[rawdata$trial_event == "tryout_rdk" | rawdata$trial_event == "rdk" | rawdata$trial_event == "practice_rdk" | rawdata$trial_event == "select_rdk", ]
...
for (i in 1:nSubjects$n){
  ...
  tryidx[which(cleandata$uniqueid == subjects$uniqueid[i] & cleandata$trial_event == "tryout_rdk")[1]:(which(cleandata$uniqueid == subjects$uniqueid[i] & cleandata$trial_event == "practice_rdk")[1]-1)] = TRUE
}
...
```{r scoreAQsurvey, echo=FALSE, warning=FALSE, message=FALSE, results="asis"}
aqdatafilename = "AQscoring.csv"
aqscoring       <- read.csv(paste(inputdir, aqdatafilename, sep="/"), header = TRUE, stringsAsFactors = FALSE)
scoremat = as.matrix(aqscoring[, 2:5])

sd = survey_data %>% select(uniqueid, responses)

subscores = matrix(nrow = nrow(sd), ncol = nrow(aqscoring))
aqscores  = matrix(nrow = nrow(sd), ncol = nrow(aqscoring))

for (i in 1:nrow(sd)){ # cycle through responses
  subSurvey = sd$responses[i]  
  colloc = as.data.frame(str_locate_all(subSurvey, ":"))
  for (j in 1:nrow(colloc)){
     subscores[i,j] = as.numeric(substr(subSurvey, start =as.numeric(colloc$start[j])+1, stop=as.numeric(colloc$start[j])+1))+1
    
     aqscores[i, j] = as.numeric(scoremat[j, subscores[i,j]])
     
  }
}

subAQ = colSums(aqscores)
