---
title: "Experiment 5b: Displayed Brightness Judgement, 4 Tasks, 500 msec Error Penalty"
...
colfile       <- read.csv(paste(inputdir,"data_dictionary_pixel_uon.csv",sep="/"), stringsAsFactors = FALSE)
datafilename = "exp5b_pixel_data_displayed.csv"
...
# OLD:
# fixed_datafilename = "210917_fixed_pix_data_uon_corrected.csv"
# random_datafilename = "210917_random_pix_data_uon_corrected.csv"
...
easyacc = rdk %>% filter(dot_difficulty == 1) %>% group_by(condition, phase, uniqueid) %>% summarise(mean = mean(correct))
...
# TODO: Rewrite using tidyverse
aov.acc.data = aggregate(list("accuracy" = rdk$correct), by = list("subject" = rdk$uniqueid, "condition" = rdk$condition, "phase" = rdk$phase, "difficulty" = rdk$dot_difficulty), FUN = function(x) mean(x, na.rm=TRUE))

aov.rt.data = aggregate(list("rt" = rdk$rt), by = list("subject" = rdk$subject, "condition" = rdk$condition, "phase" = rdk$phase, "difficulty" = rdk$dot_difficulty), FUN = function(x) mean(x, na.rm=TRUE))
...
rdk.crt = aggregate(list("crt" = rdk$rt), by = list("subject"=rdk$subject, "condition"=rdk$condition, "phase"=rdk$phase, "difficulty"=rdk$dot_difficulty, "trial"=rdk$trial_number), FUN = function(x) tail(cumsum(x), n=1))
...
pl_2 = function (dset) {
  ...
  out1 <- optim(rep(1, nitem), loglik_pl, NULL, method = "BFGS", 
    hessian = TRUE)
  test <- matrix(data = 0, nrow = factorial(nitem), ncol = nitem, 
    byrow = TRUE)
  temp1 <- 1:nitem
  i <- 1
  for (j in 1:(nitem^nitem - 1)) {
    ...
    for (k in nitem:2) {
      ...
      temp2 <- temp2 - (nitem - temp1[k]) * (nitem^(k - 
        1))
    }
    temp2 <- 0
    for (l in 1:nitem) {
  ...
  message("Chi-square residual statistic: ", round(sum(ss), 
    digits = 2), ", df: ", factorial(nitem))
...
ru.pl = random_untimed %>% rankagg() %>% pl_2() # Hessian is singular
...
for (i in 1:sum(final_n$n)){
  ...
  for (j in 0:max_trials[1]){
    ...
    nRdkEvents = tdata %>% filter(trial_event == "rdk") %>% select(dot_difficulty) %>% unique() %>% count()
