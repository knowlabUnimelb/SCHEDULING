...
# 2/11/2022 NEW: Reading data and variable names 
datafilename = "exp1_rdk_data.csv"
...
# OLD: Read fixed and random data separately
#fixed_datafilename = "exp1_fixed_rdk_data_corrected.csv"
#random_datafilename = "exp1_random_rdk_data_corrected.csv"
...
completeSubjects = completeSubjects[!(completeSubjects$subject %in% c(76732, 38904)), ]
...
# rewrite with ANOVA test
#anova_test(data = tcdata, dv = n, wid = uniqueid, within = c(phase), between = condition)
...
rr_plot <- rrdata %>% ggplot(aes(x=difficulty, y=rewardRate, fill=difficulty)) + geom_violin(adjust = 1.5, trim = TRUE, scale="width") + geom_jitter(height = 0, width = 0.05) +  stat_summary(fun=mean, geom="point", shape=23, size=2) + facet_wrap(~condition+phase) + labs(y="Reward Rate = p(Correct)/RT(sec)", x = "Difficulty") + lims(y = c(0, 2.5))
...
#sim_avg_plot <- sim_pl %>% ggplot(aes(x=avgD)) + geom_density(aes(y=..density..), n = 8, color="black", alpha=0.6, position="identity") + facet_wrap(~condition+phase) + labs(y="Probability", x = "Avg Distance")
#print(sim_avg_plot + ggtitle("Avg Distance - Plackett-Luce Model"))


print(avg_plot + geom_histogram(data = sim_pl, aes(y=..density..), bins = 6, color="black", alpha=0, position="identity") + ggtitle("Avg Distance"))
#print(avg_plot + ggtitle("Avg Distance"))
