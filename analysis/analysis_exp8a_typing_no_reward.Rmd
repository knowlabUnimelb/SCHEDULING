---
title: "2022 Schedule RT - Experiment 8a: Labelled Word List Typing, 4 Tasks - No Reward"
...
datafilename = "exp8a_typing_no_reward_data.csv"
colfile       <- read.csv(paste(inputdir,"data_dictionary_typing.csv",sep="/"), stringsAsFactors = FALSE)
...
completeSubjects = uncleandata[uncleandata$trial_number == nTotalTrials, ] %>% distinct(subject_id)
nCompleteSubjects = uncleandata[uncleandata$trial_number == nTotalTrials, ] %>% distinct(subject_id) %>% count()

# Keep only complete datasets
cleandata = uncleandata[uncleandata$subject %in% completeSubjects$subject_id, ]

# some subjects may have completed the experiment twice, keep only first completion
completionIds = cleandata %>% distinct(unique_id, subject_id)
completionCounts = cleandata %>% distinct(unique_id, subject_id) %>% count(unique_id)
...
cleandata = cleandata[cleandata$subject_id %in% keeplist$subject_id, ]
...
nUnspec  = demo_data %>% filter(sex != "CISGENDER MALE" & sex != "FEMALE" & sex != "WOMEN" & sex != "MALE" & sex != "FEMALE " & sex != "WOMAN" & sex != "MAN") %>% distinct( unique_id) %>% count()
...
```{r typing_analysis, echo=FALSE, warning=FALSE, message=FALSE, results="asis"}
...
# capture the typing RTs for the longest lists (because they're recorded as 
...
avgRT = rep(NA, nrow(typingdata),1)
nRTs = rep(NA, nrow(typingdata),1)
avgAccuracy = rep(NA, typingdata,1)
total_typing_time = rep(NA, typingdata,1)
...
options(digits=2)
typing_analysis = aggregate(list("acc" = typing_vars$acc, "rt" = typing_vars$rt, "wpm" = typing_vars$wpm, "totalTime" = typing_vars$totalTime), by = list("length" = typing_vars$nWords, "phase"=typing_vars$phase), function(x)(round(mean(x), 2)))

typing_analysis_std = aggregate(list("acc" = typing_vars$acc, "rt" = typing_vars$rt, "wpm" = typing_vars$wpm, "totalTime" = typing_vars$totalTime), by = list("length" = typing_vars$nWords, "phase"=typing_vars$phase), function(x)(round(sd(x), 2)))
