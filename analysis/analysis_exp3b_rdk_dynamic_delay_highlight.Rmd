---
title: "Experiment 3b: Dynamic RDK Direction Judgement, Highlight Selection, 4 Tasks, 500 msec Error Penalty"
...
experimentName = "dynamic_rdk_highlight" # use the index.Rmd file to track which experiment is which

# Name of the data file containing raw data
datafilename = "2021_exp3b_rdk_data_dynamic_highlight.csv" # Name of data file
...
dataDictionaryfn = "data_dictionary_ruby.csv";
...
fu = makeHeatmap(selections, "fixed_moving_delay_highlight"  , "untimed" )
fd = makeHeatmap(selections, "fixed_moving_delay_highlight"  , "deadline" )
ru = makeHeatmap(selections, "random_moving_delay_highlight" , "untimed" )
rd = makeHeatmap(selections, "random_moving_delay_highlight" , "deadline" )
```
...
unitable = uniformityTest(selections, c("fixed_moving_delay_highlight", "random_moving_delay_highlight"), c("untimed", "deadline"), fu, fd, ru, rd) 
...
#ksOutput = analyseDistances(selections, c("fixed_location_delay_highlight", "random_location_delay_highlight"), c("untimed", "deadline"))
...
fu = selections %>% 
  filter(condition == "fixed_moving_delay_highlight", phase == "untimed") %>% 
...
fd = selections %>% 
  filter(condition == "fixed_moving_delay_highlight", phase == "deadline") %>% 
  select(subject, trial, selected_difficulty_1, selected_difficulty_2, selected_difficulty_3, selected_difficulty_4) %>%
  as.matrix() %>% 
  getPLparametersBySubject()

ru = selections %>% 
  filter(condition == "random_moving_delay_highlight", phase == "untimed") %>% 
  select(subject, trial, selected_difficulty_1, selected_difficulty_2, selected_difficulty_3, selected_difficulty_4) %>%
  as.matrix() %>% 
  getPLparametersBySubject()

rd = selections %>% 
  filter(condition == "random_moving_delay_highlight", phase == "deadline") %>% 
  select(subject, trial, selected_difficulty_1, selected_difficulty_2, selected_difficulty_3, selected_difficulty_4) %>%
  as.matrix() %>% 
  getPLparametersBySubject()
```
...
#files_to_build <- list.files("analysis", pattern = "^analysis_exp3b_.*\\.Rmd$", full.names = TRUE)
