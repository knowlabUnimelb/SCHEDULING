---
title: "Experiment 5a: Pixel Brightness Judgement, 4 Tasks, 500 msec Error Penalty"
...
fixed_datafilename = "exp5a_pixel_data.csv"
...
rawdata  <- read.csv(fixed_datafn, header = FALSE, col.names = colfile$Column, colClasses = colfile$Type, stringsAsFactors = FALSE, na.strings="NA") # Add column labels to data

# odd problem with uniqueid 26323 so remove it (trials are interspersed weirdly)
rawdata = rawdata[rawdata$uniqueid != 26323, ]
...
coherence_set = sort(unique(c(cleandata$patch_0, cleandata$patch_1, cleandata$patch_2, cleandata$patch_3)), decreasing = FALSE)
...
rdk = cleandata %>% filter(trial_event == 'subtask')
...
easyacc = rdk %>% filter(dot_difficulty == 1) %>% group_by(phase, uniqueid) %>% summarise(mean = mean(correct))
...
finalSubjects = data %>% distinct(uniqueid)
...
# TODO: Rewrite using tidyverse
aov.acc.data = aggregate(list("accuracy" = rdk$correct), by = list("subject" = rdk$uniqueid, "phase" = rdk$phase, "difficulty" = rdk$dot_difficulty), FUN = function(x) mean(x, na.rm=TRUE))

aov.rt.data = aggregate(list("rt" = rdk$rt), by = list("subject" = rdk$subject, "phase" = rdk$phase, "difficulty" = rdk$dot_difficulty), FUN = function(x) mean(x, na.rm=TRUE))
...
rdk.crt = aggregate(list("crt" = rdk$rt), by = list("subject"=rdk$subject,  "phase"=rdk$phase, "difficulty"=rdk$dot_difficulty, "trial"=rdk$trial_number), FUN = function(x) tail(cumsum(x), n=1))
...
# Difficulty plot
rdk$labels = factor(rdk$dot_difficulty)
levels(rdk$labels) = c("easy", "medium", "hard", "v. hard")
...
levels(rdk.crt$labels) = c("easy", "medium", "hard", "v. hard")
...
ncounts = rdk %>% count(condition, phase, dot_difficulty)
...
# Create the table
difficulty = difficulty[c("phase", "difficulty", "n", "accuracy", "se.acc", "rt", "se.rt", "crt", "se.crt")]
...
Finally, we repeated the analysis using the cRT examining the effect of condition, phase, and difficulty. We confirmed that cRTs were shorter under a deadline, F(`r anova.crt$ANOVA$DFn[anova.crt$ANOVA$Effect == "phase"]`, `r anova.crt$ANOVA$DFd[anova.crt$ANOVA$Effect == "phase"]`) = `r round(anova.crt$ANOVA$F[anova.crt$ANOVA$Effect == "phase"], 2)`, p = `r round(anova.crt$ANOVA$p[anova.crt$ANOVA$Effect == "phase"],2)`, $\eta^2$ = `r round(anova.crt$ANOVA$ges[anova.crt$ANOVA$Effect == "phase"], 2)`, and that RTs became shorter as the RDK's became easier, F(`r anova.crt$ANOVA$DFn[anova.crt$ANOVA$Effect == "difficulty"]`, `r anova.crt$ANOVA$DFd[anova.crt$ANOVA$Effect == "difficulty"]`) = `r round(anova.crt$ANOVA$F[anova.crt$ANOVA$Effect == "difficulty"], 2)`, p = `r round(anova.crt$ANOVA$p[anova.crt$ANOVA$Effect == "difficulty"],2)`, $\eta^2$ = `r round(anova.crt$ANOVA$ges[anova.crt$ANOVA$Effect == "difficulty"], 2)`. There was again an interaction between phase and difficulty, F(`r anova.crt$ANOVA$DFn[anova.crt$ANOVA$Effect == "phase:difficulty"]`, `r anova.crt$ANOVA$DFd[anova.crt$ANOVA$Effect == "phase:difficulty"]`) = `r round(anova.crt$ANOVA$F[anova.crt$ANOVA$Effect == "phase:difficulty"], 2)`, p = `r round(anova.crt$ANOVA$p[anova.crt$ANOVA$Effect == "phase:difficulty"],2)`, $\eta^2$ = `r round(anova.crt$ANOVA$ges[anova.crt$ANOVA$Effect == "phase:difficulty"], 2)` indicating that RT decreased more with increasing dot coherence when there was no deadline compared to when there was a deadline. 
...
rr_plot <- rrdata %>% ggplot(aes(x=difficulty, y=rewardRate, fill=difficulty)) + geom_violin(adjust = 1.5, trim = TRUE) +  stat_summary(fun=mean, geom="point", shape=23, size=2) + facet_wrap(~phase) + labs(y="Reward Rate = p(Correct)/RT(sec)", x = "Difficulty") + lims(y = c(0, 2.5))
...
cn = c(rep("practice_pix", 10), rep("select_pix", 30))
...
selections = data.frame(subject = rep(finalSubjects$uniqueid, each = sdLength), phase = rep(ph, sum(final_n$n)), trial = rep(tr, sum(final_n$n)), selected_difficulty_1 = rep(NA, sum(final_n$n) * sdLength), selected_difficulty_2 = rep(NA, sum(final_n$n) * sdLength), selected_difficulty_3 = rep(NA, sum(final_n$n) * sdLength), selected_difficulty_4 = rep(NA, sum(final_n$n) * sdLength))
...
# It's unclear what pmr needs you to impute missing ranks because rankagg() removes them! 
# rankagg() is the same as as.matrix(as.tibble(fixed_untimed) %>% drop_na() %>% group_by_all() %>% count())
...
sim_avg_plot <- sim_pl %>% ggplot(aes(x=avgD, fill=phase)) + geom_histogram(aes(y=..density..), color="white", alpha=0.6, bins = 6, position="identity") + facet_wrap(~phase) + labs(y="Probability", x = "Avg Distance")
...
ks.fixed.phase1 = ks.test(allrand$s, selections %>% filter(phase == "untimed") %>% pull(avgD))$statistic/ks.test(twofix$s, selections %>% filter(phase == "untimed") %>% pull(avgD))$statistic

ks.fixed.phase2 = ks.test(allrand$s, selections %>% filter(phase == "deadline") %>% pull(avgD))$statistic/ks.test(twofix$s, selections %>% filter(phase == "deadline") %>% pull(avgD))$statistic

# Build data for plotting
randomness = as.data.frame(list("condition" = factor(c("fixed", "fixed"), levels =c("fixed")), 
                                "phase" = factor(c("untimed", "deadline"), levels = c("untimed", "deadline")), 
                                "ratio" = c(ks.fixed.phase1, ks.fixed.phase2)))
...
selection_mean_rts = selections %>% group_by(phase) %>% summarise("mrt_sel1" = round(mean(rt_1, na.rm=TRUE)), "mrt_sel2" = round(mean(rt_2, na.rm=TRUE)), "mrt_sel3" = round(mean(rt_3, na.rm=TRUE)), "mrt_sel4" = round(mean(rt_4, na.rm=TRUE))) 
...
for (i in 1:sum(final_n$n)){
  ...
  for (j in 0:max_trials[1]){
    ...
    nSelectionEvents = tdata %>% filter(trial_event == "practice_pix") %>% count()
    nRdkEvents = tdata %>% filter(trial_event == "subtask") %>% select(dot_difficulty) %>% unique() %>% count()
...
for (i in 1:sum(final_n$n)){
  ...
  for (j in 0:max_trials[2]){
    ...
    nSelectionEvents = tdata %>% filter(trial_event == "select_pix") %>% count()
    nRdkEvents = tdata %>% filter(trial_event == "subtask") %>% select(dot_difficulty) %>% unique() %>% count()
