---
title: "2022 Schedule RT - Experiment 8e: Labelled Word List Typing, 4 Tasks - Points Reward"
...
# Make sure you sort by start_date and trial_index first
```
...
datafilename = "exp8e_typing_reward_data_points.csv"
...
uncleandata = rawdata[rawdata$trial_event != "demographics" & !is.na(rawdata$trial_event), ]
survey_data = rawdata[is.na(rawdata$trial_event), ]
...
# Tryout trial not included in newcastle typing points experiment
...
#  tryidx[which(cleandata$unique_id == subjects$unique_id[i] & cleandata$trial_event == "tryout_rdk")[1]:which(cleandata$unique_id == subjects$unique_id[i] & cleandata$trial_event == "wordle_answer")[1]] = TRUE
#}
#cleandata$tryidx = tryidx
...
rt.aov = aov(rt ~ phase, data = typing_vars)
summary(rt.aov)

acc.aov = aov(acc ~ phase, data = typing_vars)
summary(acc.aov)

wpm.aov = aov(wpm ~ phase, data = typing_vars)
summary(wpm.aov)

totalTime.aov = aov(totalTime ~ phase, data = typing_vars)
summary(totalTime.aov)
...
conditions = c("high_difficulty_high_reward")
