...
experimentName = "exp2_avgtimeLabels" # use the index.Rmd file to track which experiment is which

# Name of the data file containing raw data
datafilename = "2021_exp2b_rdk_data_avgtime.csv" # Name of data file
...
dataDictionaryfn = "data_dictionary_avgTime.csv";
...
```
After removing any participants who had less than chance accuracy on the easiest RDK indicating equipment problems or a misunderstanding of task directions, there were `r final_n$n[final_n$condition == "fixed_location_delay_avgtime"]` and `r final_n$n[final_n$condition == "random_location_delay_avgtime"]` in the fixed and random location conditions, respectively.
...
fu = makeHeatmap(selections, "fixed_location_delay_avgtime"  , "untimed" )
fd = makeHeatmap(selections, "fixed_location_delay_avgtime"  , "deadline" )
ru = makeHeatmap(selections, "random_location_delay_avgtime" , "untimed" )
rd = makeHeatmap(selections, "random_location_delay_avgtime" , "deadline" )
```
...
uniformityTest(selections, c("fixed_location_delay_avgtime", "random_location_delay_avgtime"), c("untimed", "deadline"), fu, fd, ru, rd) 
```
...
fu = selections %>% 
  filter(condition == "fixed_location_delay_avgtime", phase == "untimed") %>% 
...
fd = selections %>% 
  filter(condition == "fixed_location_delay_avgtime", phase == "deadline") %>% 
  select(subject, trial, selected_difficulty_1, selected_difficulty_2, selected_difficulty_3, selected_difficulty_4) %>%
  as.matrix() %>% 
  getPLparametersBySubject()

ru = selections %>% 
  filter(condition == "random_location_delay_avgtime", phase == "untimed") %>% 
  select(subject, trial, selected_difficulty_1, selected_difficulty_2, selected_difficulty_3, selected_difficulty_4) %>%
  as.matrix() %>% 
  getPLparametersBySubject()

rd = selections %>% 
  filter(condition == "random_location_delay_avgtime", phase == "deadline") %>% 
  select(subject, trial, selected_difficulty_1, selected_difficulty_2, selected_difficulty_3, selected_difficulty_4) %>%
  as.matrix() %>% 
  getPLparametersBySubject()
```
...
#files_to_build <- list.files("analysis", pattern = "^analysis_exp2b_.*\\.Rmd$", full.names = TRUE)
