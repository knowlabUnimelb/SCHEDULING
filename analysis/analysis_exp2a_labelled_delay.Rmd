...
#  data %>% filter(uniqueid == 26545 & phase == "deadline" & condition == "random" & trial_number == 25)
...
date <- "221006"
experimentName = "exp2" # use the index.Rmd file to track which experiment is which

#define working directories
inputdir <- "data"

# NEW 2/11/2022 
datafilename = "exp2_rdk_data.csv"
...
# OLD: Reading data and variable names 
#fixed_datafilename = "exp2_fixed_rdk_delay_data_corrected.csv"
#random_datafilename = "exp2_random_rdk_delay_data_corrected.csv"
...
# Remove the following subjects because their selection phase did not record properly on too many trials
# fixed 24798 --> 81543, random 26545 --> 11054
completeSubjects = completeSubjects[!(completeSubjects$subject %in% c(81543, 11054)), ]
...
rr_plot <- rrdata %>% ggplot(aes(x=difficulty, y=rewardRate, fill=difficulty)) + geom_violin(adjust = 1.5, trim = TRUE, scale="width") +  stat_summary(fun=mean, geom="point", shape=23, size=2) + geom_jitter(height = 0, width = 0.05) + facet_wrap(~phase+condition) + labs(y="Reward Rate = p(Correct)/RT(sec)", x = "Difficulty") + lims(y = c(0, 2.5))
...
```{r writeModellingData, echo=FALSE, warning=FALSE, message=FALSE, results="hide"}
write.csv(selections, paste(inputdir, paste(date, "_", experimentName, "_", "modelling_data.csv", sep=""),sep="/"), row.names = FALSE)
